...
def validate_input_data(data):
    ...
    numeric_fields = [
        'monitoringTime', 'criticalRate', 'sithp', 
...
def calculate_critical_pressure(valve_name, sithp, k, scssv_depth=None):
    """Calcula la presión crítica según el tipo de válvula"""
    if valve_name == "SCSSV":
        if scssv_depth is None:
            raise ValueError("scssv_depth is required for SCSSV valve type")
        HPscssv = 0.067 * scssv_depth
        return 0.547 * (sithp + HPscssv)
    else:
...
def integrity_test(data):
    ...
    try:
        ...
        scssv_depth = float(data.get('scssv_depth', 0))
        ...
        critical_pressure_cavity = calculate_critical_pressure(
            valve_name, sithp, specific_heat_ratio, scssv_depth= scssv_depth
        )
