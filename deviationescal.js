...
class DeviationSystem {
  ...
  async sendDeviationToDatabase(deviation) {
    try {
      // Get the associated escalation to find the original failure
      const escalation = window.escalationSystem.escalations.find(e => e.id === deviation.escalationId);
      if (!escalation) {
        throw new Error('Associated escalation not found');
      }

      // Prepare data showing this failure now has a deviation request
      const backendData = {
        ...
        // Updated status - now has deviation
        ...
        deviationExpiryDate: deviation.expiryDate.toISOString(),
        deviationStatus: deviation.status,
        deviationCreatedBy: deviation.createdBy,
        deviationCreatedAt: deviation.createdAt.toISOString(),
        
        // Keep escalation history
        escalationId: escalation.id,
        escalatedTo: escalation.escalatedTo,
        escalationCreatedBy: escalation.createdBy,
        escalationCreatedAt: escalation.createdAt.toISOString(),
        
        // Latest update info
        updatedBy: deviation.createdBy,
        updatedAt: new Date().toISOString()
      };

      console.log('Updating failure record with deviation:', backendData);

      // Update the SAME failure record that the escalation created
      const response = await fetch(`${this.baseUrl}/integrity_test/failure?well=${encodeURIComponent(deviation.wellId)}&valve=${encodeURIComponent(deviation.valveId)}`, {
        method: 'POST',
        headers: {
      ...
      console.log('Failure status updated with deviation:', responseData);
      return responseData;

    } catch (error) {
      console.error('Failed to update failure with deviation:', error);
      showTemporaryMessage('Warning: Failed to save deviation to database', 'warning');
      return null;
    }
  }
  ...
  async updateDeviationInDatabase(deviation) {
    try {
      ...
      // Determine the new failure status based on deviation status
      let failureStatus;
      switch (deviation.status) {
        case 'approved':
          failureStatus = 'deviation-approved';
          break;
        case 'denied':
          failureStatus = 'deviation-denied';
          break;
        case 'pending':
          failureStatus = 'deviation-requested';
          break;
        default:
          failureStatus = 'deviation-requested';
      }
      ...
      const backendData = {
        ...
        // Updated failure status
        status: failureStatus,
        
        // Updated deviation details
        ...
        deviationCreatedBy: deviation.createdBy,
        deviationCreatedAt: deviation.createdAt.toISOString(),
        deviationApprovedBy: deviation.approvedBy,
        deviationApprovedAt: deviation.approvedAt ? deviation.approvedAt.toISOString() : null,
        deviationComments: deviation.comments,
        ...
        // Latest update info
        updatedBy: window.AuthService.currentUserData?.email || 'system',
        updatedAt: new Date().toISOString()
      };

      console.log('Updating failure record with deviation status:', backendData);
      ...
      console.log('Failure status updated successfully:', result);
      return result;

    } catch (error) {
      console.error('Failed to update failure with deviation status:', error);
      showTemporaryMessage('Warning: Failed to update deviation in database', 'warning');
      return null;
    }
  }
  ...
  async loadDeviationsFromDatabase(wellId = null, valveId = null) {
    try {
      console.log(deviation.wellId, deviation.valveId);
      const response = await fetch(`${this.baseUrl}/integrity_test/failure?well=${encodeURIComponent(deviation.wellId)}&valve=${encodeURIComponent(deviation.valveId)}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json'
