...
function createNotificationPanel() {
  ...
  const pendingCount = window.deviationSystem ? window.deviationSystem.getPendingDeviations().length : 0;
  const approvedCount = window.deviationSystem ? 
    window.deviationSystem.deviations.filter(d => d.status === 'approved').length : 0;
  const rejectedCount = window.deviationSystem ? 
    window.deviationSystem.deviations.filter(d => d.status === 'rejected').length : 0;
...
function loadPendingDispensations(container) {
  ...
  const pendingDeviations = window.deviationSystem.getPendingDeviations();
  ...
  pendingDeviations.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  
  const currentUserRole = window.AuthService?.currentUserData?.role || '';
  const canApprove = currentUserRole === 'admin';
  
  pendingDeviations.forEach(deviation => {
...
function loadApprovedDispensations(container) {
  ...
  const approvedDeviations = window.deviationSystem.deviations.filter(d => d.status === 'approved');
  ...
  approvedDeviations.sort((a, b) => new Date(b.approvedAt || 0) - new Date(a.approvedAt || 0));
...
function loadDeniedDispensations(container) {
  ...
  const rejectedDeviations = window.deviationSystem.deviations.filter(d => d.status === 'rejected');
  
  if (rejectedDeviations.length === 0) {
  ...
  rejectedDeviations.sort((a, b) => new Date(b.approvedAt || 0) - new Date(a.approvedAt || 0));
  
  rejectedDeviations.forEach(deviation => {
    const card = document.createElement('div');
    card.className = 'notification-card';
    card.style.cssText = `
      background: white;
...
function loadRetestRequests(container) {
  // CHANGE: Since escalations are now part of deviation system workflow
  if (!window.deviationSystem) {
    container.innerHTML = '<div style="text-align: center; padding: 20px; color: #6c757d;">System not available</div>';
    return;
  }
  
  // Get retests from failure records instead of escalation system
  const retestRequests = window.deviationSystem.deviations.filter(d => 
    d.status === 'retest-requested' || 
    (d.comments && d.comments.some(c => c.text.includes('Re-test requested')))
  );
 ...
 */
// 3. UPDATE: Modify view button event listener
function addNotificationActionListeners(container) {
  ...
  viewButtons.forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      if (type === 'deviation' && window.deviationSystem) {
        const deviation = window.deviationSystem.deviations.find(d => d.id === id);
        if (deviation) {
          window.deviationSystem.showDeviationsForEscalation(deviation.escalationId);
        }
      } else if (type === 'escalation' && window.deviationSystem) {
        // CHANGE: Handle escalation view through deviation system
        const deviation = window.deviationSystem.deviations.find(d => d.id === id);
        if (deviation) {
          // Show the failure details modal
          const well = window.wells.find(w => w.name === deviation.wellName);
          if (well && well.christmasTree && well.christmasTree.valves) {
            const valve = well.christmasTree.valves.find(v => v.id === deviation.valveId);
            if (valve) {
              const ringInfo = {
                label: valve.name || valve.id,
                valveId: deviation.valveId,
                message: deviation.description || 'Test Failed',
                failureCode: deviation.failureCode || 0
              };
              showDetailedFailureModal(ringInfo, deviation.wellName);
            }
          }
        }
      }
  ...
  approveButtons.forEach(btn => {
    btn.addEventListener('click', async (e) => {
      ...
      if (window.deviationSystem && id) {
        await window.deviationSystem.approveDeviation(id, comment);
  ...
  rejectButtons.forEach(btn => {
    btn.addEventListener('click', async (e) => {
      ...
      if (window.deviationSystem && id) {
        await window.deviationSystem.rejectDeviation(id, comment);
        // Refresh the current tab content
        loadNotificationTabContent(window.notificationPanelSettings.activeTab);
      }
    });
...
function updateNotificationCounts() {
  ...
  const approvedCount = window.deviationSystem ? 
    window.deviationSystem.deviations.filter(d => d.status === 'approved').length : 0;
  const rejectedCount = window.deviationSystem ? 
    window.deviationSystem.deviations.filter(d => d.status === 'rejected' || d.status === 'denied').length : 0;
  
  // CHANGE: Get retest count from deviation system instead of escalation system
  const retestCount = window.deviationSystem ? 
    window.deviationSystem.deviations.filter(d => 
      d.status === 'retest-requested' || 
      (d.comments && d.comments.some(c => c.text.includes('Re-test requested')))
    ).length : 0;
...
function initializeNotificationPanel() {
  ...
  if (window.deviationSystem) {
    ...
    const originalNotify = window.deviationSystem.notifyDeviationDecision;
    window.deviationSystem.notifyDeviationDecision = function(deviation, decision, comment) {
