...
class EscalationSystem {
  ...
  /**
   * [FIXED] Creates an escalation with robust well ID detection.
   */
  async createEscalation(wellName, valveId, failureDetails, userInitiated = false, wellId = null) {
    ...
    if (!wellId) {
      console.log(`Attempting to find well ID for well name: "${wellName}"`);
      const wellFromGlobal = window.wells?.find(w => w.name === wellName);
      if (wellFromGlobal && wellFromGlobal.id) {
        wellId = wellFromGlobal.id;
        console.log(`Found well ID via window.wells: ${wellId}`);
      }
      if (!wellId) {
        const wellDiv = document.querySelector(`.well-container[data-well-name="${wellName}"]`);
        if (wellDiv && wellDiv.dataset.wellId) {
          wellId = wellDiv.dataset.wellId;
          console.log(`Found well ID via DOM attribute: ${wellId}`);
        }
      }
    }

    if (!wellId) {
      console.error(`CRITICAL: Could not determine well ID for "${wellName}". Backend operations will fail.`);
    }
    ...
    const escalation = {
      ...
      wellName,
      wellId, // Now correctly populated
      valveId,
      failureDetails,
      status: 'open',
    ...
    console.log(`Created escalation ${escalation.id} for well ${wellName} (ID: ${wellId})`, escalation);
  ...
  async sendEscalationToDatabase(escalation) {
    if (!escalation.wellId) {
        console.error("Aborting sendEscalationToDatabase: wellId is missing.");
        return false;
    }
    try {
      const backendData = {
      ...
      const response = await fetch(`http://10.226.113.110:5000/api/integrity_test/failure?well=${encodeURIComponent(escalation.wellId)}&valve=${encodeURIComponent(escalation.valveId)}`, {
        method: 'POST',
        headers: {
      ...
      console.log('Escalation saved successfully:', await response.json());
      return true;
    } catch (error) {
...
// Add this new helper method to the EscalationSystem class
/**
 * Ensures an escalation has a valid wellId by attempting various lookup methods
 * @param {Object} escalation - The escalation to verify/fix
 * @returns {boolean} - True if wellId exists or was successfully recovered
 */
ensureWellId(escalation) {
  // If wellId already exists and is not null, nothing to do
  if (escalation.wellId) {
    return true;
  }
  
  console.log(`Attempting to recover missing wellId for escalation ${escalation.id}, well ${escalation.wellName}`);
  
  // Try multiple methods to find the well ID
  
  // 1. Check the global wells array
  if (window.wells && Array.isArray(window.wells)) {
    const wellObject = window.wells.find(w => w.name === escalation.wellName);
    if (wellObject && wellObject.id) {
      escalation.wellId = wellObject.id;
      console.log(`Recovered wellId from window.wells: ${escalation.wellId}`);
      return true;
    }
  }
  
  // 2. Try to find it in the DOM by well name attribute
  const wellElements = document.querySelectorAll('[data-well-name]');
  for (const element of wellElements) {
    if (element.dataset.wellName === escalation.wellName && element.dataset.wellId) {
      escalation.wellId = element.dataset.wellId;
      console.log(`Recovered wellId from DOM: ${escalation.wellId}`);
      return true;
    }
  }
  
  // 3. Special case for wells with numeric names (like C19) - use name as ID
  if (/^[A-Z]\d+$/.test(escalation.wellName)) {
    // This is likely a well with a pattern like C19, use the name as ID
    escalation.wellId = escalation.wellName;
    console.log(`Using well name as ID for ${escalation.wellName}`);
    return true;
  }
  
  // If we reach here, we couldn't recover the well ID
  console.error(`Failed to recover wellId for well ${escalation.wellName}`);
  return false;
}

// Modify acknowledgeEscalation to ensure wellId exists
async acknowledgeEscalation(id, comment = '') {
  const esc = this.escalations.find(e => e.id === id);
  if (esc) {
    // Ensure we have a valid wellId before proceeding
    if (!esc.wellId) {
      this.ensureWellId(esc);
    }
    ...
    // Log the escalation before sending to verify wellId exists
    console.log(`About to update escalation in database:`, {
      id: esc.id,
...
// Also modify updateEscalationInDatabase to have a final fallback
async updateEscalationInDatabase(escalation) {
  try {
    // Try to recover wellId if missing
    if (!escalation.wellId) {
      if (!this.ensureWellId(escalation)) {
        // If we still don't have a wellId, use the wellName as a last resort
        console.warn(`Using wellName as fallback ID for ${escalation.wellName}`);
        escalation.wellId = escalation.wellName;
      }
    }
  ...
  async _sendRetestRequestToDatabase(escalation, comment) {
    if (!escalation.wellId) {
        console.error("Aborting _sendRetestRequestToDatabase: wellId is missing.");
        return false;
    }
    try {
      const backendData = {
        ...
        wellId: escalation.wellId,
        valveId: escalation.valveId,
        decisionBy: window.AuthService.currentUserData?.email || 'system',
        comment: comment,
      ...
      console.log('Re-test request sent successfully.');
      return true;
    } catch (error) {
      console.error(`Failed to send re-test request for escalation ${escalation.id}:`, error);
      return false;
    }
  }
  ...
  async requestRetest(id, comment = '') {
    ...
    if (esc) {
      ...
      this.updateDashboardEscalationIndicators();
      await this._sendRetestRequestToDatabase(esc, comment);
      console.log(`Re-test requested for escalation ${id}`);
    }
  }
  
  // All other methods (getPriorityFromTrafficLight, determineSuperior, etc.) remain unchanged.
  // ... Paste the rest of your helper methods from the original file here ...


// Initialize escalation system

// ... (rest of the EscalationSystem class is unchanged)
