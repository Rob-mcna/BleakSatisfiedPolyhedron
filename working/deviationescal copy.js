...
class DeviationSystem {
  ...
  saveToLocalStorage() {
    try {
      const dataToSave = this.deviations.map(dev => ({
        ...dev,
        createdAt: dev.createdAt.toISOString(),
        approvedAt: dev.approvedAt ? dev.approvedAt.toISOString() : null,
        expiryDate: dev.expiryDate.toISOString()
      }));
      localStorage.setItem('deviation_backup', JSON.stringify(dataToSave));
      console.log('Deviations saved to localStorage backup');
    } catch (error) {
  ...
  loadFromLocalStorage() {
    try {
      const data = localStorage.getItem('deviation_backup');
      if (data) {
        const parsed = JSON.parse(data);
        return parsed.map(dev => ({
          ...dev,
          createdAt: new Date(dev.createdAt),
          approvedAt: dev.approvedAt ? new Date(dev.approvedAt) : null,
          expiryDate: new Date(dev.expiryDate),
  ...
  clearLocalStorage() {
    try {
      localStorage.removeItem('deviation_backup');
    } catch (error) {
  ...
  // Prepare deviation data in the same format as escalation endpoint expects
  _prepareDeviationForBackend(deviation) {
    ...
    return {
      ...
      expiryDate: deviation.expiryDate.toISOString(),
      ...
      approvedAt: deviation.approvedAt ? deviation.approvedAt.toISOString() : null,
      comments: deviation.comments,
      failureCode: escalation.failureDetails?.failureCode || 'DEVIATION',
      matrixCategory: escalation.failureDetails?.matrixCategory || 'Dispensation'
    };
  }

  // Send deviation data to the backend using the same endpoint as escalations
  async sendDeviationToDatabase(deviation) {
    try {
      const backendData = this._prepareDeviationForBackend(deviation);
      if (!backendData) {
        console.error('Cannot prepare deviation for backend - escalation not found');
        this.saveToLocalStorage(); // Save locally as backup
        return false;
      }

      const response = await fetch(`http://10.226.112.1:5000/api/integrity_test/failure?well=${encodeURIComponent(deviation.wellId)}&valve=${encodeURIComponent(deviation.valveId)}`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        console.error(`Deviation save failed: ${response.status}`, errorText);
        this.saveToLocalStorage(); // Save locally as backup
        return false;
      }

      console.log('Deviation saved successfully to database');
      this.saveToLocalStorage(); // Always maintain local backup
      return true;
    } catch (error) {
  ...
  /**
   * Loads all deviations from the database with retry logic and better error handling
   */
  async loadDeviationsFromDatabase() {
    if (this.isLoaded) {
      console.log('Deviations already loaded, skipping');
      return;
    }

    this.loadAttempts++;
    console.log(`Loading deviations from database (attempt ${this.loadAttempts}/${this.maxLoadAttempts})`);

    try {
      // Load from localStorage first as immediate fallback
      const localDeviations = this.loadFromLocalStorage();
      if (localDeviations.length > 0) {
        localDeviations.forEach(dev => {
          if (!this.deviations.find(d => d.id === dev.id)) {
            this.deviations.push(dev);
          }
        });
        console.log(`Loaded ${localDeviations.length} deviations from localStorage`);
        this.updateDashboardDeviationIndicators();
      }

      // Now try to load from database and merge
      const response = await fetch(`http://10.226.112.1:5000/api/integrity_test/deviation?well=${encodeURIComponent(deviation.wellId)}&valve=${encodeURIComponent(deviation.valveId)}`,
  {
        method: 'GET',
        // headers: { 'Content-Type': 'application/json' }, // Often not needed for GET
  }
);
...
const data = await response.json();
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error(`Failed to load deviations from database: ${response.status}`, errorText);
        
        // If we have local data, use it and mark as loaded
        if (localDeviations.length > 0) {
          this.isLoaded = true;
          return;
        }
        ...
        if (this.loadAttempts < this.maxLoadAttempts) {
          ...
          setTimeout(() => this.loadDeviationsFromDatabase(), 2000);
          return;
        } else {
          console.error('Max load attempts exceeded, giving up database load');
          this.isLoaded = true; // Prevent further attempts
          return;
        }
      }

      const failureRecords = await response.json();

      // Filter records that have deviation data
      const deviationRecords = failureRecords.filter(r => r.deviationId || (r.failureCode === 'DEVIATION'));
      let loadedCount = 0;

      // Reconstruct local deviations from database
      deviationRecords.forEach(record => {
        try {
          // Extract deviation data from the record
          const deviation = {
            id: record.deviationId || `DEV-${Date.now()}-${Math.random()}`,
            escalationId: record.escalationId,
            wellName: record.wellId, // Adjust based on your data structure
            wellId: record.wellId,
            valveId: record.valveId,
            status: record.status || 'pending',
            description: record.description || '',
            justification: record.justification || '',
            expiryDate: new Date(record.expiryDate),
            createdBy: record.createdBy || 'unknown',
            creatorRole: record.creatorRole || 'unknown',
          ...
          // Avoid duplicates
          if (!this.deviations.find(d => d.id === deviation.id)) {
            this.deviations.push(deviation);
            loadedCount++;
            console.log(`Loaded deviation from DB: ${deviation.id}`);
          }
        } catch (error) {
          console.error('Error processing deviation record:', error, record);
        }
      });

      // Save the loaded data to localStorage for future use
      if (loadedCount > 0) {
        this.saveToLocalStorage();
      }

      // Mark as loaded and refresh UI
      this.isLoaded = true;
      this.updateDashboardDeviationIndicators();
      console.log(`Successfully loaded ${loadedCount} deviations from database`);
      ...
      if (this.loadAttempts < this.maxLoadAttempts) {
        ...
        setTimeout(() => this.loadDeviationsFromDatabase(), 3000);
      } else {
        console.error('Max load attempts exceeded, using localStorage data only');
        this.isLoaded = true;
      }
    }
  }
  ...
  async initializeWithRetry() {
    ...
    const waitForSystems = () => {
      return new Promise((resolve) => {
        const checkSystems = () => {
          ...
          const escalationSystemReady = window.escalationSystem && window.escalationSystem.escalations;
          const authServiceReady = window.AuthService && window.AuthService.currentUserData;
          
          if (escalationSystemReady || waitAttempts >= maxWaitAttempts) {
            console.log(`Systems ready check: escalations=${!!escalationSystemReady}, auth=${!!authServiceReady}, attempts=${waitAttempts}`);
            resolve();
          } else {
    ...
    await this.loadDeviationsFromDatabase();
  }

  // Create a new deviation request with enhanced error handling
  async createDeviation(escalationId, description, justification, expiryDate) {
    ...
    // Send to database (async, don't wait for it)
    this.sendDeviationToDatabase(deviation).catch(error => {
      console.error('Failed to sync deviation to database:', error);
    });

    return deviation;
  }

  // Approve a deviation request with enhanced persistence
  async approveDeviation(deviationId, comment = '') {
    ...
    if (deviation) {
      ...
      this.sendDeviationToDatabase(deviation).catch(error => {
        console.error('Failed to sync approved deviation to database:', error);
      });
  ...
  // Deny a deviation request with enhanced persistence
  async denyDeviation(deviationId, comment = '') {
    ...
    if (deviation) {
      ...
      // Sync to database (async)
      this.sendDeviationToDatabase(deviation).catch(error => {
        console.error('Failed to sync denied deviation to database:', error);
      });
  ...
  addCommentToDeviation(deviationId, comment) {
    ...
    if (deviation && comment) {
      ...
      // Sync to database
      this.sendDeviationToDatabase(deviation).catch(error => {
        console.error('Failed to sync comment to database:', error);
      });
    }
  }
  ...
  updateDashboardDeviationIndicators() {
    ...
    if (typeof addDeviationIndicatorsToWells === 'function') {
      addDeviationIndicatorsToWells();
    }
  ...
  // [Rest of your existing methods: notifyDeviationDecision, refreshOpenModalsWithDeviationUpdate, 
  //  showDeviationRequestForm, showDeviationsForEscalation, showDeviationAdminPanel, 
  //  showWellDeviationSummary - keep them exactly as they are]
  ...
  refreshOpenModalsWithDeviationUpdate(deviation) {
    ...
    if (failureDetailModal) {
      const escalation = window.escalationSystem?.escalations?.find(e => e.id === deviation.escalationId);
      if (escalation) {
        const ringInfo = window.lastRingInfo;
  ...
  showDeviationRequestForm(escalationId) {
    ...
    modal.innerHTML = `
      ...
      <div style="margin-bottom: 15px; background: #f8f9fa; padding: 12px; border-radius: 4px; border-left: 4px solid #007bff;">
        ...
        <p style="margin: 5px 0 0; font-size: 0.9em; color: #666;"><strong>Escalation:</strong> ${escalation.id}</p>
      </div>
...
/**
 * Loads all deviations from the database and restores them into the local system.
...
// Load existing deviations from the database when the app starts

window.deviationSystem = window.deviationSystem || new DeviationSystem();
document.addEventListener('DOMContentLoaded', () => {
  setTimeout(() => {
    if (window.deviationSystem && typeof window.deviationSystem.loadDeviationsFromDatabase === 'function') {
      window.deviationSystem.loadDeviationsFromDatabase();
    }
  }, 1500); // Slight delay to ensure AuthService and wells are ready
});
// Initialize the deviation system
