...
function filterUsers(users, searchTerm) {
  ...
  return users.filter(user => {
    ...
    const pickUp = (user.pickUp || "").toLowerCase();
    const status = (user.status || "").toLowerCase();
    const role = getUserRoleText(user).toLowerCase();

    return [email, pickUp, status, role].some(value => value.includes(term));
  });
}
      ...
      ${filteredUsers.map(user => `
        ...
        ">
          ...
          <div style="margin-top:6px;font-size:12px;color:#6b7280;display:flex;gap:10px;flex-wrap:wrap;">
            <span><b>Pick Up:</b> ${user.pickUp || "-"}</span>
            <span><b>Status:</b> ${user.status || "-"}</span>
            <span><b>Role:</b> ${getUserRoleText(user)}</span>
          </div>
        </div>
      `).join("")}
    </div>
  ...
  section.innerHTML = `
    ...
    ">
      ...
      ">
        <div style="margin-bottom:22px;">
          <h1 style="margin:0;font-size:28px;color:#111827;">Create User</h1>
          <div style="margin-top:6px;color:#6b7280;font-size:14px;">Aligned with current backend roles.</div>
        </div>
