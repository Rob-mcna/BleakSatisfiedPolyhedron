...
from app.models.cavity_volume import CavityVolume
from app.models.integrity_test_result import InegrityTestResult
from app.models.integrity_test_failure import InegrityTestFailure
...
def validate_input_data(data):
    ...
    required_fields = [
        ...
        'sithp', 'pressureUpstreamPFSV', 'liquidYield', 'initialPressure',
...
def calculate_critical_pressure(valve_name, sithp, pressure_upstream_pfsv, k):
    ...
    if valve_name == "SCSSV":
        return 0.547 * (sithp + pressure_upstream_pfsv)
    else:
        xc = (2 / (k + 1)) ** (k / (k - 1)) if (k + 1) != 0 and (k - 1) != 0 else 0
        return pressure_upstream_pfsv * xc
...
def integrity_test(data):
    ...
    try:
        ...
        specific_heat_ratio = float(data.get("k", 1.3))
...
def give_integrity_test_failure(well_id, valve_id):
    inegrity_test_failure = InegrityTestFailure.query.options(
        db.joinedload(InegrityTestFailure.test_well),
        db.joinedload(InegrityTestFailure.test_valve)
    ).filter(
        InegrityTestFailure.test_well_id == well_id,
        InegrityTestFailure.test_valve_id == valve_id,
    ).first()
...
def save_test_failure(well_id, valve_id, data):
    try:
        ...
        existing_record = InegrityTestFailure.query.filter_by(
                test_well_id=well_id,
