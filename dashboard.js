...
// Fetch valve test results for all wells and all valves using the per-valve API endpoint
...
async function fetchValveTestResults() {
    try {
        ...
        for (const { wellName, valveId, data } of resultsArray) {
            // Optional: log every raw API response (can be verbose)
            // console.log(`[API RESPONSE] Well ${wellName}, valve ${valveId}:`, data);

            let isValid = false;

            // Good case: Single object with test keys (not an error)
            if (data && typeof data === "object" && !Array.isArray(data)) {
                if (!("Error" in data || "error" in data)) {
                    isValid = true;
                }
            }
            // Good case: Array of test objects (if ever returned)
...
async function fetchWellsAndSyncResults() {
  ...
  try {
    const wellsResponse = await fetch('http://127.0.0.1:5000/api/wells/');
    if (!wellsResponse.ok) {
