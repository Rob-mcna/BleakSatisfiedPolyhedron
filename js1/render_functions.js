...
async function loadValveTestStatuses(wellId) {
  ...
  try {
    ...
    if (window.valveTestResults && (window.valveTestResults[wellId] || (wellConfig.name && window.valveTestResults[wellConfig.name]))) {
    ...
    const response = await fetch(`http://10.226.14.79:5000/api/integrity_test/results?well=${wellId}`);
    
    // *** START OF FIX ***
    // Gracefully handle 404 (Not Found) as a valid case meaning "no tests found"
    if (!response.ok) {
      if (response.status === 404) {
        console.log(`No test results found for well ${wellId} (404), returning empty object.`);
        // Store an empty object to prevent re-fetching
        if (!window.valveTestResults) window.valveTestResults = {};
        window.valveTestResults[wellId] = {};
        if (wellConfig.name) window.valveTestResults[wellConfig.name] = {};
        return {};
      }
      // For other errors (500, etc.), still show a warning
