...
async function showValveInputPanel(currentWellId, currentValveId, valveName) {
  ...
  try {
    ...
    const response = await fetch(`http://10.226.112.213:5000/api/integrity_test/blocking_valve?well=${currentWellId}&valve=${currentValveId}`);
    if (response.ok) {
  ...
  if (currentForm) {
    ...
    currentForm.onsubmit = async function (e) {
      ...
      if (submitMsgEl) {
        ...
        const response = await fetch('http://10.226.112.213:5000/api/integrity_test/', {
            method: 'POST',
...
async function refreshWellDataFromAPI(wellId = null) {
  try {
    const response = await fetch('http://10.226.112.213:5000/api/wells/');
    if (!response.ok) {
      return false;
    }
    ...
    if (wellId) {
      const testResultsResponse = await fetch(`http://10.226.112.213:5000/api/integrity_test/results?well=${wellId}`);
